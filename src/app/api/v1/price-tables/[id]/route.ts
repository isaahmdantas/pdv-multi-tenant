...
import { updatePriceTableSchema } from "@/modules/pricing/schemas";
...
export const GET = withApiGuards(async (ctx, _request, routeCtx: RouteCtx) => {
  ...
  const priceTable = await new PriceTableService(prisma).get(ctx, id);
...
export const PUT = withApiGuards(async (ctx, request, routeCtx: RouteCtx) => {
  ...
  const parsed = updatePriceTableSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(
      {
        error: {
  ...
  const updated = await new PriceTableService(prisma).update(ctx, id, parsed.data, {
    ip: clientIp(request),
    device: request.headers.get("user-agent") ?? undefined,
  });

  return NextResponse.json({ priceTable: updated });
}, "pricing.manage");

export const DELETE = withApiGuards(async (ctx, request, routeCtx: RouteCtx) => {
  const { id } = await routeCtx.params;
  await new PriceTableService(prisma).deactivate(ctx, id, {
    ip: clientIp(request),
    device: request.headers.get("user-agent") ?? undefined,
  });
