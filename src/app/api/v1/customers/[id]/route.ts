...
import { updateCustomerSchema } from "@/modules/customers/schemas";
...
export const GET = withApiGuards(async (ctx, _request, routeCtx: RouteCtx) => {
  ...
  const customer = await new CustomerService(prisma).get(ctx, id);
...
export const PUT = withApiGuards(async (ctx, request, routeCtx: RouteCtx) => {
  ...
  const updated = await new CustomerService(prisma).update(ctx, id, parsed.data, {
    ip: clientIp(request),
...
export const DELETE = withApiGuards(async (ctx, request, routeCtx: RouteCtx) => {
  const { id } = await routeCtx.params;
  await new CustomerService(prisma).deactivate(ctx, id, {
    ip: clientIp(request),
    device: request.headers.get("user-agent") ?? undefined,
  });
