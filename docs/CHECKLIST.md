...
| F11-09 | Cancelamento | ✅ | F11-04 | `SaleService.cancel`, `src/app/api/v1/sales/[id]/cancel/route.ts`, `src/components/sales/cancel-sale-button.tsx`, `src/app/(app)/vendas/page.tsx` | integração sales.test.ts (reverte estoque IN + audit + 409 na 2ª vez) + smoke | ação visual no histórico /vendas (botão em 2ª confirmação, perm `sales.cancel`); cancel só de COMPLETED — SUSPENDED → 409 SALE_NOT_CANCELLABLE (evita devolver estoque que nunca saiu) |
| F11-10 | Suspensão | ✅ | F11-04 | `SaleService.suspend`, `src/app/api/v1/sales/suspend/route.ts`, `suspendSaleSchema`, `constants.ts` (SUSPENDED), `AUDIT_ACTIONS` (SALE_SUSPENDED), `src/components/pdv/pdv-shell.tsx` | integração sales.test.ts + smoke | cria Sale SUSPENDED com items/preços congelados, SEM StockMovement nem CashMovement; botão "Suspender venda" no PDV (limpa carrinho, item volta só se retomado); funciona sem sessão de caixa aberta |
| F11-11 | Recuperação | ✅ | F11-10 | `SaleService.recover`/`discard`, `api/v1/sales/suspended` (GET), `api/v1/sales/[id]/recover` e `[id]/discard` (POST), `src/components/pdv/suspended-sales.tsx`, `vendas` (badge Suspensa + filtro Suspensas) | integração sales.test.ts + smoke | painel "Vendas suspensas" no PDV: Retomar recarrega carrinho (preços re-resolvidos) e remove o rascunho; Descartar remove sem efeitos; recuperar/discartar 2ª vez → 409 SALE_NOT_SUSPENDED; listagem por `status=SUSPENDED` |
